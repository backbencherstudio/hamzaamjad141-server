...
exports.toDeActiveUser = exports.toActiveUser = exports.overview = exports.membership = exports.getAllPilotUser = void 0;
const client_1 = require("@prisma/client");
const client_2 = require("@prisma/client");
const prisma = new client_2.PrismaClient();
const getAllPilotUser = async (req, res) => {
    ...
    try {
        ...
        const transformedUsers = users.map((user) => {
            const homeBase = user.Weather.find((w) => w.status === "HOMEBASE")?.location;
            const favorites = user.Weather.filter((w) => w.status === "FAVURATE").map((w) => w.location);
...
exports.getAllPilotUser = getAllPilotUser;
const membership = async (req, res) => {
    try {
        ...
        const filteredUsers = users.filter(user => status === "ACTIVE" || status === "DEACTIVE"
...
const toActiveUser = async (req, res) => {
    const { id } = req.params;
    try {
...
exports.toActiveUser = toActiveUser;
const toDeActiveUser = async (req, res) => {
    const { id } = req.params;
    try {
        const user = await prisma.user.findUnique({
            where: { id },
...
exports.toDeActiveUser = toDeActiveUser;
