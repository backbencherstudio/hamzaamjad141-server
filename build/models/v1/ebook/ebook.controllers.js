...
exports.deleteEbook = exports.updateEbook = exports.searchEbooks = exports.getAllebook = exports.createEbooks = void 0;
const client_1 = require("@prisma/client");
const path_1 = __importDefault(require("path"));
const base_utl_1 = require("../../../utils/base_utl");
const fs_1 = __importDefault(require("fs"));
const prisma = new client_1.PrismaClient();
const createEbooks = async (req, res) => {
    ...
    try {
        ...
        const missingFile = ["pdf", "cover"].find((fileType) => !req.files[fileType]);
        if (missingFile) {
            res.status(400).json({
                success: false,
        ...
        const responseData = {
            ...portcusts,
            pdf: (0, base_utl_1.getImageUrl)(`/uploads/${pdfFile.filename}`),
...
exports.createEbooks = createEbooks;
const getAllebook = async (req, res) => {
    try {
        ...
        const ebooksWithUrls = ebooks.map((ebook) => ({
            ...ebook,
            pdf: (0, base_utl_1.getImageUrl)(`/uploads/${ebook.pdf}`),
            cover: (0, base_utl_1.getImageUrl)(`/uploads/${ebook.cover}`),
        }));
        res.status(200).json({
            success: true,
        ...
        res.status(500).json({
            success: false,
            message: error instanceof Error ? error.message : "Failed to fetch ebooks",
        });
    }
};
exports.getAllebook = getAllebook;
        ...
        const whereClause = searchQuery
            ? {
                title: {
                    ...
                    mode: 'insensitive' // Cast to Prisma's QueryMode type
                }
            }
            : {};
        const [ebooks, totalEbooks] = await Promise.all([
...
exports.searchEbooks = searchEbooks;
...
const updateEbook = async (req, res) => {
    ...
    try {
        ...
        if (req.files) {
            ...
            if (pdfFile) {
                const oldPdfFilePath = path_1.default.join(__dirname, "../../../uploads", ebook.pdf);
                if (fs_1.default.existsSync(oldPdfFilePath)) {
                    fs_1.default.unlinkSync(oldPdfFilePath); // Delete old PDF file
                }
                pdfFileUrl = pdfFile.filename;
                updateData.pdf = pdfFileUrl; // Use the correct field name
            }
            if (coverFile) {
                const oldCoverFilePath = path_1.default.join(__dirname, "../../../uploads", ebook.cover);
                if (fs_1.default.existsSync(oldCoverFilePath)) {
                    fs_1.default.unlinkSync(oldCoverFilePath); // Delete old cover image
                }
                coverFileUrl = coverFile.filename;
        ...
        const responseData = {
            ...updatedEbook,
            pdf: (0, base_utl_1.getImageUrl)(`/uploads/${updatedEbook.pdf}`),
            cover: (0, base_utl_1.getImageUrl)(`/uploads/${updatedEbook.cover}`),
        };
        res.status(200).json({
            success: true,
...
const deleteEbook = async (req, res) => {
    ...
    try {
        ...
        const pdfFilePath = path_1.default.join(__dirname, "../../../uploads", ebook.pdf);
        const coverFilePath = path_1.default.join(__dirname, "../../../uploads", ebook.cover);
...
//# sourceMappingURL=ebook.controllers.js.map